...
import { ArrowLeft, Package, Truck, CheckCircle, Clock, AlertCircle, MapPin, Phone, Calendar, User } from 'lucide-react';
...
const TrackOrder = () => {
  ...
  const fetchOrder = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/suppliers/orders/${orderId}`);
  ...
  const updateOrderStatus = async (newStatus) => {
    try {
      ...
      await api.put(`/suppliers/orders/${orderId}/status`, { status: newStatus });
          ...
          <div className="card">
            ...
            {canUpdateStatus(order.status) && (
              <div className="mb-6 p-4 bg-blue-50 rounded-lg">
                <h3 className="font-medium text-blue-900 mb-3">Update Order Status</h3>
            ...
            <div className="space-y-4">
              {[
                { status: 'pending', label: 'Order Received', description: 'Order has been placed by vendor' },
                { status: 'accepted', label: 'Order Accepted', description: 'Order has been accepted for processing' },
                { status: 'preparing', label: 'Preparing Order', description: 'Order is being prepared for delivery' },
                { status: 'in-transit', label: 'In Transit', description: 'Order is on its way to vendor' },
                { status: 'delivered', label: 'Delivered', description: 'Order has been delivered successfully' }
              ].map((step, index) => {
                const isCompleted = ['pending', 'accepted', 'preparing', 'in-transit', 'delivered'].indexOf(step.status) <= ['pending', 'accepted', 'preparing', 'in-transit', 'delivered'].indexOf(order.status);
              ...
              {order.notes?.vendor && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">{order.notes.vendor}</p>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="space-y-3">
                ...
                {order.vendor.phone && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">{order.vendor.phone}</span>
                  </div>
                )}
              </div>
            </div>
